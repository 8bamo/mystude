...
<main id="main" class="pt-[84px]">
    ...
    <section class="border-b border-black bg-black py-20 text-center text-white">
        <div class="mx-auto w-full max-w-5xl px-5 sm:px-8">
            <span class="font-cta text-xs font-bold uppercase tracking-[0.18em] text-white/70">Kooperationen & Anfragen</span>
            <h1 class="mt-4 font-display text-5xl leading-[1.1] text-white sm:text-6xl">Mit Stuttgart arbeiten.</h1>
            <p class="mx-auto mt-5 max-w-3xl text-base leading-8 text-white/72">mystu.de verbindet Stadtkultur, Produkte und echte Geschichten. Wenn du Stuttgart liebst und Reichweite mitbringst, lass uns reden.</p>
        </div>
    </section>

    <section class="border-b border-black/15 bg-[#eef1f0] py-16">
        <div class="mx-auto grid w-full max-w-7xl gap-4 px-5 sm:px-8 md:grid-cols-3">
            <div class="rounded-[8px] border border-black/15 bg-white p-6 text-center"><span class="block font-display text-4xl text-black">100%</span><span class="mt-3 block text-sm leading-7 text-muted">Stuttgart-Fokus, kein generischer Content</span></div>
            <div class="rounded-[8px] border border-black/15 bg-white p-6 text-center"><span class="block font-display text-4xl text-black">2</span><span class="mt-3 block text-sm leading-7 text-muted">starke Kategorien: Geschenke und Kultur</span></div>
            <div class="rounded-[8px] border border-black/15 bg-white p-6 text-center"><span class="block font-display text-4xl text-black">48h</span><span class="mt-3 block text-sm leading-7 text-muted">Rückmeldung auf jede Anfrage</span></div>
        </div>
    </section>
    ...
    <section class="py-20">
        <div class="mx-auto w-full max-w-7xl px-5 sm:px-8">
            <div class="mb-10 flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
                <div>
                    <span class="font-cta text-xs font-bold uppercase tracking-[0.18em] text-muted">Aktuelle Partner</span>
                    <h2 class="mt-4 font-display text-4xl leading-[1.1] text-ink sm:text-5xl">Menschen, mit denen wir gerade arbeiten.</h2>
                </div>
            ...
            <div class="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
                <?php foreach ($current_partners as $partner): ?>
                    <a href="<?php echo esc_url($partner['url']); ?>" target="_blank" rel="noopener" class="group peak-card rounded-[8px] border border-black/15 bg-white p-4 transition">
                        <?php if (!empty($partner['image'])): ?>
                            <?php $image_class = (($partner['image_fit'] ?? 'cover') === 'contain') ? 'object-contain bg-white p-5' : 'object-cover'; ?>
                            <img src="<?php echo esc_url($partner['image']); ?>" alt="<?php echo esc_attr($partner['name']); ?>" class="aspect-[4/5] w-full rounded-[4px] <?php echo esc_attr($image_class); ?>" loading="lazy">
                        <?php else: ?>
                        ...
                        <p class="mt-3 font-cta text-[0.72rem] font-bold uppercase tracking-[0.12em] text-muted"><?php echo esc_html($partner['role']); ?></p>
                        <p class="mt-3 font-sans text-sm font-bold text-black"><?php echo esc_html($partner['handle']); ?></p>
                        <span class="mt-6 inline-flex text-sm font-bold text-muted transition group-hover:text-black">Auf Instagram ansehen -></span>
                    </a>
                <?php endforeach; ?>
            </div>
        </div>
    ...
    <section class="py-20">
        <div class="mx-auto w-full max-w-7xl px-5 sm:px-8">
            <div class="grid gap-4 lg:grid-cols-2">
                <article class="rounded-[8px] border border-black bg-black p-8 text-white">
                    <div class="font-mono text-sm text-white/70">01</div>
                    <h2 class="mt-6 font-display text-3xl text-white">Creators & Influencer</h2>
                    <p class="mt-4 text-base leading-8 text-white/75">Du hast eine Community, die Stuttgart kennt und liebt? Wir suchen Creator für echte Kooperationen mit Substanz.</p>
                    <ul class="mt-6 space-y-3 text-sm leading-7 text-white/70">
                    ...
                    <a href="#collab-form" class="mt-8 inline-flex min-h-[3rem] items-center justify-center rounded border border-white bg-white px-6 py-3 font-cta text-sm font-bold uppercase tracking-[0.08em] text-black transition">Kooperation anfragen</a>
                </article>
                <article class="rounded-[8px] border border-black/15 bg-white p-8">
                    ...
                    <a href="#collab-form" class="mt-8 inline-flex min-h-[3rem] items-center justify-center rounded border border-black bg-white px-6 py-3 font-cta text-sm font-bold uppercase tracking-[0.08em] text-black transition">Presseanfrage stellen</a>
                </article>
            </div>
        </div>
    </section>

    <section class="border-y border-black/15 bg-[#eef1f0] py-20">
        <div class="mx-auto grid w-full max-w-7xl gap-10 px-5 sm:px-8 lg:grid-cols-[1fr_auto] lg:items-start">
            <div>
                <span class="font-cta text-xs font-bold uppercase tracking-[0.18em] text-muted">Warum mystu.de</span>
            ...
            <div class="grid gap-6 lg:text-right">
                <div><span class="block font-display text-5xl text-black">0711</span><span class="mt-2 block font-cta text-sm uppercase tracking-[0.14em] text-muted">Stuttgarts Vorwahl, unsere DNA</span></div>
                <div><span class="block font-display text-5xl text-black">∞</span><span class="mt-2 block font-cta text-sm uppercase tracking-[0.14em] text-muted">Themen rund um Stuttgart</span></div>
            </div>
        </div>
    </section>
    ...
    <section class="py-20" id="collab-form">
        <div class="mx-auto w-full max-w-4xl px-5 text-center sm:px-8">
            <span class="font-cta text-xs font-bold uppercase tracking-[0.18em] text-muted">Kontakt</span>
            ...
            <div class="mt-10 rounded-[8px] border border-black/15 bg-white p-8 sm:p-10">
                ...
                <form class="space-y-5 text-left" method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>">
                    ...
                    <button type="submit" class="inline-flex min-h-[3rem] items-center justify-center rounded border border-black bg-black px-6 py-3 font-cta text-sm font-bold uppercase tracking-[0.08em] text-white transition">Anfrage senden</button>
                </form>
            </div>
        </div>
    </section>
